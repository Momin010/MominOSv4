...
import { History, Trash2, Calculator, Settings } from "lucide-react"
...
export default function CalculatorApp() {
  ...
  const Button = ({ onClick, className = "", children, variant = "number", ...props }: {
    ...
    const baseClasses = "h-16 rounded-xl font-light text-xl transition-all duration-200 flex items-center justify-center select-none"
    
    const variantClasses = {
      number: "glass-button text-white hover:bg-white/20",
      operator: `${operation === children ? 'bg-white/30 text-orange-500' : 'glass-button bg-orange-500/20 text-white'} hover:bg-orange-500/30`,
    ...
    <div className="h-full bg-black/20 backdrop-blur-xl p-6 flex flex-col">
      ...
      {isScientific && (
        <div className="grid grid-cols-4 gap-2 mb-4">
          ...
          <Button onClick={() => scientificFunction("sqrt")} variant="scientific">√</Button>
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <Button onClick={() => setDisplay(String(-parseFloat(display)))} variant="function">±</Button>
        ...
        <Button onClick={performCalculation} variant="operator">=</Button>
      </div>
    </div>
  )
}
